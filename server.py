...
def todos_update(todo_id):
    ...
    for todo in todos:
        if todo['id'] == int(todo_id):
            todo['completed'] = request.json['completed'] if 'completed' in request.json else todo['completed']
            todo['text'] = request.json['text'] if 'text' in request.json else todo['text']
            res = todo
